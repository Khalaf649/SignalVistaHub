 ...
 */
export async function fetchDopplerPlotData(
  params: DopplerParams
): Promise<DopplerPlotData> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch plot data: ${response.status} ${response.statusText}`
    );
  }
 ...
 */
export async function predictDoppler(
  audioFile: File
): Promise<DopplerPrediction> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to predict: ${response.status} ${response.statusText}`
    );
  }
