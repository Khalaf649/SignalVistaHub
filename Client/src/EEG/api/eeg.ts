 ...
 */
export async function preprocessEDF(
  file: File
): Promise<EEGPreprocessResponse> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to preprocess EEG: ${response.status} ${response.statusText}`
    );
  }
...
export async function fetchEEGData(accessUrl: string): Promise<EEGData> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch EEG data: ${response.status} ${response.statusText}`
    );
  }
