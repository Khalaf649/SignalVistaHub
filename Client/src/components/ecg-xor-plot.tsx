...
import LinearSignalPlot from "./plots/linear-plot";
...
export default function XorECGPlot({
  ...
  useEffect(() => {
    ...
    const signal1 = recording.signals.map(
      (sample: number[]) => sample[channel1]
    );
    const signal2 = recording.signals.map(
      (sample: number[]) => sample[channel2]
    );
  ...
  if (!recording || !recording.signals || recording.signals.length === 0) {
    return (
      <div className="text-muted-foreground">No recording data available</div>
    );
  }
