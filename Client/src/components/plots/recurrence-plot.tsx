...
export default function RecurrencePlot({
  ...
  useEffect(() => {
    ...
    const histogram: number[][] = Array.from({ length: numBins }, () =>
      Array(numBins).fill(0)
    );
    for (let i = 0; i < samples; i++) {
      const binX = Math.floor(
        ((data1[i] - minX) / (maxX - minX)) * (numBins - 1)
      );
      const binY = Math.floor(
        ((data2[i] - minY) / (maxY - minY)) * (numBins - 1)
      );
  ...
  return (
    <div className="w-full flex flex-col items-center gap-2">
      <canvas
        ...
        width={600}
        height={600}
        className="border border-border rounded-lg bg-black shadow-lg"
      />
    </div>
  );
}
